...
/**
 * Función para hacer login de administrador
...
export const loginAdministrador = async (email, password) => {
  try {
    ...
    if (!adminDoc.exists()) {
      throw new Error('Datos de administrador no encontrados');
    }
...
export const eliminarAdministradorConAuth = async (adminId) => {
  try {
    ...
    if (adminDoc.exists() && adminDoc.data().esAdminPrincipal) {
    ...
    // Nota: Eliminar de Firebase Auth requiere permisos especiales o el usuario autenticado
